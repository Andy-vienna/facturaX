...
public class FxHtmlEditor extends JPanel {
    ...
    private static final String HTML_TEMPLATE = """
		...
		<meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Editor</title>
		<style>
		  ...
		  blockquote { border-left:4px solid #ccc; margin:8px 0; padding:8px 12px; color:#ba55d3; }
		...
		(function(){
		  ...
		  function cmd(name, value=null){ ed.focus(); document.execCommand(name,false,value); ed.dispatchEvent(new Event('input',{bubbles:true})); }
		  function getHtml(){ return ed.innerHTML; }
		  function setHtml(html){ ed.innerHTML = html || ''; placeCaretEnd(ed); }
		  function placeCaretEnd(el){ el.focus(); const r=document.createRange(); r.selectNodeContents(el); r.collapse(false); const s=window.getSelection(); s.removeAllRanges(); s.addRange(r); }
		  ...
		  ed.addEventListener('keydown', e=>{ if(e.ctrlKey && !e.shiftKey && !e.altKey){ if(e.key==='b'||e.key==='B'){e.preventDefault();cmd('bold');} if(e.key==='i'||e.key==='I'){e.preventDefault();cmd('italic');} if(e.key==='u'||e.key==='U'){e.preventDefault();cmd('underline');} }});
